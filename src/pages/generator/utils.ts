function joinZerosLeft(generatedString: string, requiredLength: number) {
...
function getRandomNumber(range: number, min: number, requiredLength: number) {
  return joinZerosLeft(String(Math.floor((Math.random() * range) + min)), requiredLength);
}

export function generateFLInn() {
...
export function generateSnils() {
  ...
  let control = String(9 * (+result[0]) + 8 * (+result[1]) + 7 * (+result[2]) +
    ...
    3 * (+result[6]) + 2 * (+result[7]) + (+result[8]));
  if (+control > 101) {
